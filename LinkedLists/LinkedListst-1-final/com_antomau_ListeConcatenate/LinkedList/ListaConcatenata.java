...
public class ListaConcatenata {
	...
	/**
	 * Questo metodo aggiunge un nodo in coda alla lista.
	 * @param s Lo studente da aggiungere come oggetto del nodo in coda alla lista
	 */
	...
	/**
	 * Questo metodo rimuove il nodo in coda alla lista.
	 */
	public void removeLast() {
		...
		while(cur.link != null) //fintanto che il nodo corrente ha un nodo successivo
		{
			previous = cur; //il nodo precedente diviene il nodo corrente
			cur = cur.link; //il nodo corrente diviene il nodo successivo a quello corrente
		} //end of while
		previous.link = null; //il riferimento al nodo successivo del nodo corrente diviene nullo: perde ogni riferimento al nodo corrente
	} //end of removeLast
	...
	/**
	 * Questo metdo restituisce una stringa adatta a diventare il testo di una JLabel che stampa tutto il contenuto della lista
	 * @return Una stringa da impostare come testo di una JLabel che stampa l'intero contenuto della lista
	 */
	public String toJLabelString()
	{
		if (headList==null) throw new NoSuchElementException();
		String result = "";
		result += "<html><table border=\"1\"><tr><th>Surname</th><th>Name</th><th>Serial Number</th><th>Exam</th><th>Exam Votation</th></tr>";
		Node current = headList;
		while (current!=null)
		{
			result += "<tr><td>" + current.student.getSurname() + "</tr><td>" + current.student.getName() + "</td><td>" + current.student.getSerial() + "</td><td>" + current.student.getExamName() + "</td><td>" + current.student.getExamVote() + "</td>";
			current = current.link;
		} //end of while
		result += "</table></html>";
		return result;
	} //end of toJLabelString
